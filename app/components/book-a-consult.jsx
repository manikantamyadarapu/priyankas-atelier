...
export default function BookAConsult() {
  ...
  useEffect(() => {
    ...
    // Force pure black — override any inherited background
    ...
    // Smooth bidirectional zoom — scrub tracks scroll both ways
    gsap.timeline({
      scrollTrigger: {
        trigger: section,
        start: "top 95%",
        end: "top 15%",
        scrub: 1.5,           // higher = smoother lag (feels weighted)
      },
    }).to(section, {
      scale: 1,
      opacity: 1,
      borderRadius: "12px",
      ease: "power1.inOut",  // smooth both entering AND leaving
    });
  ...
  /* ── Canvas letter animation (unchanged) ── */
  useEffect(() => {
    ...
    function init()    { resize(); letters = []; for (let i = 0; i < 130; i++) letters.push(new Letter(true)); }
    function animate() { ctx.clearRect(0, 0, W, H); letters.forEach(l => { l.update(); l.draw(); }); animId = requestAnimationFrame(animate); }
